...
type: execute
depends_on: []
files_modified: [package.json, pnpm-workspace.yaml, tsconfig.base.json, .gitignore, .npmrc, packages/shared/package.json, packages/shared/tsconfig.json, packages/shared/src/index.ts]
---
...
<task type="auto">
  <name>Task 4: Scaffold packages/shared workspace</name>
  <files>packages/shared/package.json, packages/shared/tsconfig.json, packages/shared/src/index.ts</files>
  <action>
    ...
    Create packages/shared/src/index.ts with placeholder:
    - Export empty object or common types: export const API_VERSION = 'v1';

    Run: cd packages/shared && pnpm install
  </action>
  <verify>cd packages/shared && pnpm build succeeds, dist/ folder created with index.js and index.d.ts</verify>
  <done>packages/shared workspace scaffolded with TypeScript build configured, ready for shared types and constants</done>
</task>
...
- [ ] packages/shared workspace exists with package.json, tsconfig.json, src/index.ts
- [ ] `cd packages/shared && pnpm build` succeeds
...
- packages/shared scaffolded and builds successfully
