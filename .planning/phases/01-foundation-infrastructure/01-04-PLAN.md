...
files_modified: [infra/docker-compose.yml, infra/postgres/init.sql, apps/backend/.env.example, apps/stylipp.com/.env.example]
---

<objective>
Create Docker Compose infrastructure with PostgreSQL, Qdrant, and Redis for local development.

Purpose: Establish containerized database and vector store infrastructure that both frontend and backend depend on. This provides a consistent local development environment.
...
**Production**: Deferred (no prod/dev split yet)
</context>
...
<task type="auto">
  ...
  <verify>cat infra/postgres/init.sql shows uuid-ossp extension and UTC timezone</verify>
  <done>PostgreSQL initialization script created with uuid-ossp extension and UTC timezone</done>
</task>

<task type="auto">
  <name>Task 3: Configure connection strings and environment variables</name>
  <files>apps/backend/.env.example, apps/stylipp.com/.env.example</files>
  <action>
    ...
    Create apps/stylipp.com/.env.example:
  ...
  <verify>apps/backend/.env.example and apps/stylipp.com/.env.example exist with connection strings; .gitignore contains .env pattern</verify>
...
- [ ] init.sql extensions created: `psql -c "\dx"` shows uuid-ossp
</verification>
...
- PostgreSQL has uuid-ossp extension
...
- Set up PostgreSQL 16 with uuid-ossp extension
