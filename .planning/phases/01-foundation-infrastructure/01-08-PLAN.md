...
**Pre-commit scope**: ESLint, Prettier, ruff, black on staged files only (requires backend venv active)
  ...
  <action>
    ...
    **frontend-lint:**
    ...
    - steps:
      ...
      - cd apps/stylipp.com && pnpm lint (ESLint)
      - cd apps/stylipp.com && pnpm type-check (add script: "type-check": "tsc --noEmit")
...
<task type="auto">
  ...
  <files>.lintstagedrc.json, apps/stylipp.com/package.json</files>
  <action>
    Install lint-staged:
    `pnpm add -D -w lint-staged`

    Install ESLint and Prettier for frontend:
    `cd apps/stylipp.com && pnpm add -D eslint @typescript-eslint/parser @typescript-eslint/eslint-plugin prettier eslint-config-prettier`

    Create .lintstagedrc.json at root (assumes backend venv is active when committing):
    ```json
    {
      "apps/stylipp.com/**/*.{ts,tsx}": [
        "pnpm --filter web exec eslint --fix",
        "pnpm --filter web exec prettier --write"
      ],
      "apps/stylipp.com/**/*.{json,css,md}": [
        "pnpm --filter web exec prettier --write"
      ],
      "apps/backend/**/*.py": [
    ...
    Create apps/stylipp.com/.eslintrc.json:
    - extends: ["eslint:recommended", "plugin:@typescript-eslint/recommended", "prettier"]
    - parser: @typescript-eslint/parser
    - plugins: ["@typescript-eslint"]

    Create apps/stylipp.com/.prettierrc:
    - semi: false
    - singleQuote: true
    - trailingComma: "es5"
    - printWidth: 100

    Update apps/stylipp.com/package.json scripts:
...
- [ ] ESLint and Prettier installed in apps/stylipp.com
- [ ] Test commit triggers pre-commit hook and runs linters
- [ ] Push to GitHub triggers CI workflow (check Actions tab)
- [ ] apps/stylipp.com/package.json has lint, format, type-check scripts
</verification>
...
- `apps/stylipp.com/.eslintrc.json` - ESLint configuration for frontend
- `apps/stylipp.com/.prettierrc` - Prettier configuration
- Updated `package.json` - Added prepare, lint-staged scripts
- Updated `apps/stylipp.com/package.json` - Added lint, format, type-check scripts
