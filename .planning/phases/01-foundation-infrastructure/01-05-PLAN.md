...
files_modified: [apps/backend/src/features/auth/router/router.py, apps/backend/src/features/auth/service/service.py, apps/backend/src/features/auth/schemas/schemas.py, apps/backend/src/features/auth/utils/jwt.py, apps/backend/src/models/user.py, secrets/jwt/private.pem, secrets/jwt/public.pem, apps/backend/src/core/dependencies.py]
---
...
**Feature folder pattern**: features/auth/router/router.py, features/auth/service/service.py, features/auth/schemas/schemas.py, features/auth/utils/jwt.py
...
<task type="auto">
  ...
  <action>
    ...
    Add secrets/ directories to .gitignore to prevent committing keys:
...
<task type="auto">
  ...
  <files>apps/backend/src/features/auth/utils/jwt.py, apps/backend/src/features/auth/schemas/schemas.py, apps/backend/src/models/user.py</files>
  <action>
    Create apps/backend/src/features/auth/utils/jwt.py:
    - Import jwt from python-jose[cryptography]
    - Load private.pem and public.pem from secrets/jwt/ directory (check ../secrets/jwt/ for running from src/)
    ...
    Create apps/backend/src/features/auth/schemas/schemas.py:
    ...
    Create apps/backend/src/models/user.py:
    - User model extending Base from src.models.base
  ...
  <done>JWT utility functions created with ES256 algorithm in src/features/auth/, auth schemas defined, User model created in src/models/</done>
</task>

<task type="auto">
  <name>Task 3: Implement auth router with protected route example</name>
  <files>apps/backend/src/features/auth/router/router.py, apps/backend/src/features/auth/service/service.py, apps/backend/src/main.py, apps/backend/src/core/dependencies.py</files>
  <action>
    Create apps/backend/src/core/dependencies.py:
    - Function get_current_user(token: str = Depends(OAuth2PasswordBearer(tokenUrl="/api/auth/login"))) -> UUID
      - Calls verify_token(token) from src.auth.utils.jwt
      - Returns user_id
      - Used as dependency for protected routes

    Create apps/backend/src/features/auth/service/service.py:
    ...
    Create apps/backend/src/features/auth/router/router.py:
    ...
    - Import auth router from src.auth.router.router
    - app.include_router(auth_router, prefix="/api")
    - Example protected route: GET /api/auth/me requires valid JWT

    Create empty __init__.py files in src/features/auth/, src/features/auth/router/, src/features/auth/service/, src/features/auth/schemas/, src/features/auth/utils/
  </action>
  ...
  <done>Auth router created with protected /me endpoint in src/features/auth/router/, get_current_user dependency implemented, feature folder structure follows convention</done>
</task>
...
- [ ] secrets/ AND apps/backend/secrets/ both in .gitignore
...
- [ ] Feature folder structure follows convention (src/features/auth/router/router.py, src/features/auth/service/service.py)
...
- `apps/backend/src/features/auth/router/router.py` - Auth API routes
- `apps/backend/src/features/auth/service/service.py` - Auth business logic
- `apps/backend/src/features/auth/schemas/schemas.py` - Auth request/response schemas
- `apps/backend/src/features/auth/utils/jwt.py` - JWT token utilities
- `apps/backend/src/models/user.py` - User database model
- `apps/backend/src/core/dependencies.py` - get_current_user dependency
- Updated `apps/backend/src/main.py` - Include auth router
