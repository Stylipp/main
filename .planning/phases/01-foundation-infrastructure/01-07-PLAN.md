...
<objective>
Configure Traefik reverse proxy for Cloudflare Tunnel routing (single compose file).

Purpose: Establish reverse proxy layer that routes Cloudflare Tunnel traffic to the correct services on the server via Traefik.
Output: Traefik running in Docker, routes configured for stylipp.com (frontend), api.stylipp.com (backend), qdrant.stylipp.com (admin).
</objective>
...
**Domain**: stylipp.com
**Routing plan**:
- stylipp.com → apps/stylipp.com (React PWA)
...
<task type="auto">
  <name>Task 1: Add Traefik service to docker-compose.yml</name>
  <files>infra/docker-compose.yml, infra/traefik/traefik.yml</files>
  <action>
    Update infra/docker-compose.yml to add Traefik:
...
<task type="auto">
  ...
  <action>
    ...
    **Add web service (React PWA):**
    - build: ../apps/stylipp.com (will create Dockerfile in later phase)
    - For now, use placeholder: image: nginx:alpine with volume mount to apps/stylipp.com/dist
    - labels:
      - traefik.enable=true
