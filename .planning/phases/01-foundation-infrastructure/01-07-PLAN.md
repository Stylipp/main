...
files_modified: [infra/docker-compose.yml, infra/traefik/traefik.yml, infra/traefik/dynamic.yml]
---

<objective>
Configure Traefik reverse proxy with Docker labels for routing frontend, backend, and Qdrant through existing Cloudflare Tunnel.

Purpose: Establish reverse proxy layer that routes traffic from your Cloudflare Tunnel to appropriate services (stylipp.com → web, api.stylipp.com → backend). Traefik provides automatic service discovery via Docker labels and handles internal routing.
Output: Traefik running in Docker, routes configured for stylipp.com (frontend), api.stylipp.com (backend), qdrant.stylipp.com (admin), services accessible through tunnel.
</objective>
...
<task type="auto">
  <name>Task 1: Add Traefik service to Docker Compose</name>
  <files>infra/docker-compose.yml, infra/traefik/traefik.yml</files>
  <action>
    Update infra/docker-compose.yml to add Traefik service:
