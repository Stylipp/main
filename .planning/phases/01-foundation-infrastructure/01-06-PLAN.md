...
type: execute
depends_on: ["01-03"]
files_modified: [apps/backend/src/storage/service/service.py, apps/backend/src/storage/schemas/schemas.py, apps/backend/src/config.py, apps/backend/.env.example, apps/backend/src/storage/router/router.py]
---
...
<task type="auto">
  ...
  <files>apps/backend/src/config.py, apps/backend/.env.example</files>
  <action>
    Update apps/backend/src/config.py to add object storage configuration:
  ...
  <verify>apps/backend/src/config.py has S3_* configuration fields, .env.example contains PLACEHOLDER credentials (not real ones)</verify>
...
<task type="auto">
  ...
  <files>apps/backend/src/storage/service/service.py, apps/backend/src/storage/__init__.py</files>
  <action>
    Create apps/backend/src/storage/service/service.py:
    ...
    Create __init__.py files in src/storage/, src/storage/service/
  </action>
  <verify>No syntax errors in src/storage/service/service.py, aioboto3 imports resolve correctly</verify>
...
<task type="auto">
  ...
  <files>apps/backend/src/storage/schemas/schemas.py, apps/backend/src/storage/router/router.py, apps/backend/src/main.py</files>
  <action>
    Create apps/backend/src/storage/schemas/schemas.py:
    - UploadResponse(BaseModel): url (HttpUrl), key (str), size (int)

    Create apps/backend/src/storage/router/router.py:
    ...
    - Import storage router from src.storage.router.router
    - app.include_router(storage_router, prefix="/api")

    Test connection: Visit /api/storage/health to verify Hetzner credentials work

    Create __init__.py in src/storage/router/, src/storage/schemas/
  </action>
...
- [ ] apps/backend/src/config.py has S3_ENDPOINT_URL and credentials config
